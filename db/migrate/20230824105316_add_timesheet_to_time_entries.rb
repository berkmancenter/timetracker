class AddTimesheetToTimeEntries < ActiveRecord::Migration[7.0]
  def change
    ...
    add_reference :periods, :timesheet, null: false, foreign_key: true, default: default_timesheet.id
    change_column_default :time_entries, :timesheet_id, from: default_timesheet.id
    change_column_default :periods, :timesheet_id, from: default_timesheet.id
  end
end
